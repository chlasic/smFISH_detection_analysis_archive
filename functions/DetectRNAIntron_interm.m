    ...
    if ~isempty(A) && sum(sum(A)) ~= 0
        for wsz = [2 3 4]
            for j=1:length(A(:,1))
                ...
                if wsz == 2
                    if cal(j,1) > 1*10^-(mupr * 1) || cal(j,2) < mupr * 1.2 || cal(j,3) < mupr * 0.9
                        B(A(j,1),A(j,2)) = 0;
                        caller = 0;
                    end
                elseif wsz == 3
                    if cal(j,1) > 1*10^-(mupr * 2) || cal(j,2) < mupr * 1.3
                        B(A(j,1),A(j,2)) = 0;
                        caller = 0;
                    end
                elseif wsz == 4
                    if cal(j,1) > 5*10^-(mupr * 2) || cal(j,2) < mupr * 1.4
                        B(A(j,1),A(j,2)) = 0;
                        caller = 0;
                    end
                end
