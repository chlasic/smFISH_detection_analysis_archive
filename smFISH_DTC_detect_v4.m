...
%%%%%%------------ Manual input -------------------------------
%%% -------  for Windows
MASTER_Image_path = 'C:\maleDTC_SLC\'; % <<<<<-- Change the path!!!  Master image folder (one folder above the folder containing the image file to process).
...
OUTPUTdir1=  'Arg1-DTC male_Lng_new';
...
ChOrder = [        1      ];   %%% !!! List only RNA channels !!!
...
%         for f = [16 17 18 19 20 29]   % for male   
%         for f = 11:15
...
%         iminfo = [iminfo; temp];
        ...
        PhaPla = lifdat{1,1}(chainfo(:) == find(Chinfo(:,2) == 4), :);
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%                 prna{ChOrder == 2}(prna{ChOrder == 2}(:,13) == 999,:) = [];  
        ...
        %% Detect DTC by P(lag-2)::myr-GFP
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        % Seperate planes (green channel)
        % The last number on line below determines which color channel to use.
        AbGPla = lifdat{1,1}(chainfo(:) == find(Chinfo(:,2) == 6), :);
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        zproj = cat(3, AbGPla{:,1});
        mipDTC = max(zproj, [], 3);
        

        %%% "DTCmask" contains: (unit: pixels)
        %%% (1) x-coor | (2) y-coor | (3) z-coor | (4) radius of the cell on that point.| 
        DTCmask = DetectDTC(AbGPla, iminfo, pix, 4);          %%% mupr = 0.1-1 for most raw image settings.
        
        %%% visualize detection
%         figure
%         for k = 1:size(DTCmask,1)
%             imshow(DTCmask{k,1}*10)
%             pause
%         end
